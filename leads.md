...
- **TRI (Thomson Reuters)** — Down ~23.2% on adjusted yfinance pricing on 2026-06-16, still below the price gate after capital-return/share-consolidation effects. Raw-price comparisons can overstate the decline; monitor only if the adjusted decline deepens materially past ~30% and legal-AI pressure is tied to operating weakness.
- **DOCU (DocuSign)** — Roughly flat/slightly above pre-ChatGPT (+0.8% on 2026-06-16). Citi downgrade and e-signature commoditization thesis are plausible only if price crosses materially lower.
- **SFIX (Stitch Fix)** — Above pre-ChatGPT (+9.3% on 2026-06-16). AI styling could threaten the human stylist model if fundamentals deteriorate further, but it no longer clears even a basic below-baseline screen.
